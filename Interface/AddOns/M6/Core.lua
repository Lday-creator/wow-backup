...
local EV, AB, RW, KR = T.Evie do
	...
	RW = T.ActionBook:compatible("Rewire", 1, 21)
...
function api:HasProfile(character, realm, spec)
...
function api:ReplaceProfileMap(character, realm, spec)
	return replaceProfiledElement("slots", character, realm, spec)
end
function api:ReplaceProfileBindings(character, realm, spec)
